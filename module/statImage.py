...
def generateMMRHistoryImage(userId: int, x: dict, tier: str):
	# 샨티 님의 RP 변동 이력
	data : dict = x['result']['3']
...
def generateStatImage(user: list, x: dict):
	...
	foreground = Image.new("RGBA", (image.width, image.height), (0, 0, 0, 0))
	tierFullImage = Image.open(f"image/Tier/Full/{tier[2].capitalize()}.png").convert("RGBA")
	foreground.paste(tierFullImage, (130, 425), tierFullImage)
	image = Image.alpha_composite(image, foreground)

	# generateMMRHistoryImage(userId, {}, tier[2]) # TODO
