...
def WeaponTxtKo(id):
	conn = sqlite3.connect("ERData/Weapon.db", isolation_level=None)
	c = conn.cursor()
	c.execute('SELECT ko FROM Weapon WHERE id=?', (id,))
	a = c.fetchone()
	if a[0] != 'None':
		return a[0]
	else:
...
def WeaponName(id):
	conn = sqlite3.connect("ERData/Weapon.db", isolation_level=None)
	c = conn.cursor()
	c.execute('SELECT name FROM Weapon WHERE id=?', (id,))
	a = c.fetchone()
	if a[0] != 'None':
		return f"{a[0]}"
	else:
...
def getSkinUrl(skinId: int):
	return f"https://aya.gg/media/images/characters/full/{getSkinUrl_String(skinId)}.png"
