...
class CustomGame_JoinCheck(disnake.ui.View):
	...
	async def confirm(self, btn: disnake.ui.Button, i: disnake.MessageInteraction):
		...
		await i.response.edit_message(content="✅ 참여가 완료되었어요.", view=None)
		self.stop()
	...
	async def cancel(self, btn: disnake.ui.Button, i: disnake.MessageInteraction):
		await i.response.edit_message(content="✅ 참여를 취소했어요.", view=None)
		self.stop()
...
class CustomGame(commands.Cog):
	...
	async def createCustom_slashCommand(
		...
		open_user: bool = commands.Param(
			...
			description=disnake.Localized(
				"Disclose the information of participant? (If it's True, general users CANNOT check participants in the custom game.)",
				data={
		...
		roomId = createCustomGame(i.user.id, i.guild.id, name, gameType, open, open_user)
		await i.edit_original_message(
			embed=disnake.Embed(
				title="✅ 내전 생성 완료!",
				description=f"일련번호: `{roomId}`",
				color=0x00ff00
			))
